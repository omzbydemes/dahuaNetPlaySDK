...
//            ToolKits.writeLog("AudioDataCallBack received " + byAudioFlag);
    ...
    private boolean startAudioRecord()	{
        ...
        ///调用PLAYSDK库，来开始录制音频
        boolean bOpenRet = IPlaySDK.PLAYOpenStream(nPort, null, 0, 1024 * 1024) != 0;
        if(bOpenRet) {
            boolean bPlayRet = IPlaySDK.PLAYPlay(nPort, (Surface) null) != 0;
            if(bPlayRet) {
                ...
                boolean bSuccess = IPlaySDK.PLAYOpenAudioRecord(cbAudioRecord, mTalkFormatList.type[0].nAudioBit,
                        mTalkFormatList.type[0].dwSampleRate, nFrameLength, 0) != 0;
                if(bSuccess) {
                    bRet = true;
